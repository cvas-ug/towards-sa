...
data_dir = '20190301' # Balanced training data
                      # Total evaluation data (12337 baxter + 2499 env)

#data_dir = '20190311' # Total 99 in /eval/baxter has 99 data of baxter(synced)
                       # Total 99 in /eval/env (49 pro and images unsynced by swapping(which represent (not me)) + (50 env)

#data_dir = '20190313'   #/val/baxter has baxter (99 synced)
                        #/val/env has only baxter(47 unsynced by sawpping + zeros all velocity manually).

#data_dir = '20190325'   #/val/baxter has baxter (100 synced)
                        #/val/env has only baxter(100 unsynced by randomize the velocity part only).

#data_dir = '20190328'   #/val/baxter has baxter (100 synced)
                        #/val/env has baxter(100 synced).
...
class Arc3(nn.Module):
    def __init__(self):
        super(Arc3, self).__init__()
        self.fc1new = nn.Linear(19,4)

        self.fc2 = nn.Linear(19, 8)
        self.fc2new = nn.Linear(8, 4)
    ...
    def forward(self, inputFCFromResnet, pro):
        ...
        # x = x.view(-1, 18 * 28 * 28)
        #x = x.view(-1, 3456)
        x = self.fc1new(inputFCFromResnet)
        ###x = F.relu(x)
        #x = x.view(-1, 2)
        # check if needed
        #x = F.relu(self.fc1(x))
...
def train_model(model, conmodel, criterion, optimizer, scheduler, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if phase == 'train':
                        ...
                        if batch_id % 100 == 0:
                            ...
                            print('batch #{} of {}, this Loss: {:.6f}'.format(batch_id, dataset_sizes['train'], loss.item()))
                        
                        if (batch_id % dataset_sizes['train'] == 0) and (batch_id != 0):
                            print('[epoch #%d   %d] average loss: %.6f' % (epoch+1, batch_id+1, closs/dataset_sizes['train']))
                            train_closs_average.append(closs/dataset_sizes['train'])
                            closs = 0 
                    if phase == 'val':    
                        if (batch_id % dataset_sizes['val'] == 0) and (batch_id != 0): 
                            val_closs_average.append(closs/dataset_sizes['val'])
                            closs = 0                       
            ...
            if phase == 'train':
                #val_closs_average.append(running_loss/dataset_sizes['train']) # equivalent to epoch_loss during train phase
                train_accuracy_average.append(epoch_acc.tolist())

            if phase == 'val':
                #val_closs_average.append(running_loss/dataset_sizes['val']) # equivalent to epoch_loss during val phase
                val_accuracy_average.append(epoch_acc.tolist())
    ...
    plt.figure()
    #plt.plot(t1, train_closs_average, val_closs_average,train_accuracy_average, val_accuracy_average)# ,label='epoch'+str(epoch))
    plt.plot(t1, train_closs_average, 'r', val_closs_average, 'b', train_accuracy_average, 'g', val_accuracy_average, 'purple' )# ,label='epoch'+str(epoch))
    plt.legend(['Train loss ave.', 'Val loss ave.', 'Train accu ave.', 'Val accu ave.'])
    ...
    plt.title("Arch3 training and val losses")
    print("---")
    print(train_closs_average)
    plt.show()
...
def accuracy(model):
    ...
    #when predicting self, how often it is correct.
    ...
    # npv = tn / (tn+fn)
    #print("precision - Negative predictive value (NPV) = {}".format(npv))
...
if __name__ == '__main__':
    ...
    model_arc3 = train_model(model_ft, conmodel, criterion, optimizer_ft, exp_lr_scheduler, num_epochs=25)
