...
data_dir = '20190514-case3' #case3
#data_dir = '20190514-case4' #case4
...
""" class arch3model(nn.Module): #2conves
    def __init__(self):
    ...
    def forward(self, image, proprioception):
        ...
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        outof_img = self.fc3(x)
        
        x = torch.cat((outof_img, proprioception), dim=1)
        x = F.relu(self.fc4(x))
        x = self.fc5(x)
        return x """
class arch3model(nn.Module):   #4coves
    def __init__(self):
        super(arch3model, self).__init__()
        self.conv1 = nn.Conv2d(3, 128, 5)
        self.pool = nn.MaxPool2d(2, 2)
