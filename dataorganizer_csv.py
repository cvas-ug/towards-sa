...
with open(DATASET + ".csv", mode='w', newline='', encoding='utf-8') as dataset_file:
    ...
    for name in base_names:
        img_right_path = data_path +"/"+ DATASET + "/images_right/" + name + ".jpg"

        img_left_path = data_path +"/"+ DATASET + "/images_left/" + name + ".jpg"
        img_left_path = img_left_path.replace("imageright","imageleft")

        img_disparity = data_path +"/"+ DATASET + "/images_disparity/" + name + ".yaml"
        img_disparity = img_disparity.replace("imageright","disparity")

        pro_path = data_path +"/"+ DATASET + "/pro/" + name + ".yaml"
